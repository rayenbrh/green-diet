        ...
        <div className="mt-10 grid grid-cols-2 gap-3 sm:grid-cols-3 md:gap-4">
          {picks.map((p, i) => (
            <SectionReveal key={p.id} delay={i * 0.05}>
              <div className="group relative aspect-square overflow-hidden rounded-[14px]" style={{ backgroundColor: p.bgColor }}>
                <div className="flex h-full flex-col items-center justify-center p-4 text-center">
                  <span className="text-5xl">{p.emoji}</span>
                  <p className="mt-3 font-cormorant text-lg text-deep/90">
